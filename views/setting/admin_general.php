...
echo $form->field($model, 'site_type', ['template' => '{label}<div class="col-md-6 col-sm-9 col-xs-12">{input}{error}</div>'])
...
echo $form->field($model, 'site_oauth', ['template' => '{label}<div class="col-md-6 col-sm-9 col-xs-12">{input}{error}</div>'])
...
<div id="construction" <?php echo $model->online == '1' ? 'style="display: none;"' : ''; ?>>
	...
	echo $form->field($model, 'construction_date', ['template' => '{label}<div class="col-md-6 col-sm-9 col-xs-12">{input}{error}</div>'])
		->textInput(['type' => 'date'])
...
echo $form->field($model, 'event_i', ['template' => '{label}<div class="col-md-6 col-sm-9 col-xs-12">{input}{error}</div>'])
...
<div id="events" <?php echo $model->event_i == '0' ? 'style="display: none;"' : ''; ?>>
	...
	echo $form->field($model, 'event_startdate', ['template' => '{label}<div class="col-md-6 col-sm-9 col-xs-12">{input}{error}</div>'])
		->textInput(['type' => 'date'])
	...
	echo $form->field($model, 'event_finishdate', ['template' => '{label}<div class="col-md-6 col-sm-9 col-xs-12">{input}{error}</div>'])
	->textInput(['type' => 'date'])
		->label($model->getAttributeLabel('event_finishdate'), ['class'=>'control-label col-md-3 col-sm-3 col-xs-12']); ?>

	<?php echo $form->field($model, 'event_tag', ['template' => '{label}<div class="col-md-6 col-sm-9 col-xs-12">{input}{error}<span class="small-px">'.Yii::t('app', 'tambahkan tanda koma (,) jika ingin menambahkan event tag lebih dari satu.').'</span></div>'])
		->textarea(['rows'=>2,'rows'=>6])
		->label($model->getAttributeLabel('event_tag'), ['class'=>'control-label col-md-3 col-sm-3 col-xs-12']); ?>
</div>

<?php echo $form->field($model, 'site_title', ['template' => '{label}<div class="col-md-6 col-sm-9 col-xs-12">{input}{error}<span class="small-px">'.Yii::t('app', 'Give your community a unique name. This will appear in the &lt;title&gt; tag throughout most of your site.').'</span></div>'])
	->textInput(['maxlength' => true])
	->label($model->getAttributeLabel('site_title'), ['class'=>'control-label col-md-3 col-sm-3 col-xs-12']); ?>

<?php echo $form->field($model, 'site_url', ['template' => '{label}<div class="col-md-6 col-sm-9 col-xs-12">{input}{error}</div>'])
	->textInput(['maxlength' => true])
	->label($model->getAttributeLabel('site_url'), ['class'=>'control-label col-md-3 col-sm-3 col-xs-12']); ?>

<?php echo $form->field($model, 'site_description', ['template' => '{label}<div class="col-md-6 col-sm-9 col-xs-12">{input}{error}<span class="small-px">'.Yii::t('app', 'Enter a brief, concise description of your community. Include any key words or phrases that you want to appear in search engine listings.').'</span></div>'])
...
<?php echo $form->field($model, 'general_include', ['template' => '{label}<div class="col-md-6 col-sm-9 col-xs-12">{input}{error}<span class="small-px">'.Yii::t('app', 'Anything entered into the box below will be included at the bottom of the <head> tag. If you want to include a script or stylesheet, be sure to use the &lt;script&gt; or &lt;link&gt; tag.').'</span></div>'])
	->textarea(['rows'=>2,'rows'=>6])
