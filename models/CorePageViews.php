...
class CorePageViews extends \app\components\ActiveRecord
{
	...
	public function init()
	{
		...
		$this->templateColumns['pageName'] = [
			...
			'visible' => !Yii::$app->request->get('page') ? true : false,
		];
		...
		$this->templateColumns['views'] = [
			...
			'value' => function($model, $key, $index, $column) {
				$views = $model->views;
				return Html::a($views, ['page/view-detail/manage', 'view'=>$model->primaryKey], ['title'=>Yii::t('app', '{count} views', ['count'=>$views]), 'data-pjax'=>0]);
			},
			'filter' => false,
			'contentOptions' => ['class'=>'text-center'],
			'format' => 'raw',
