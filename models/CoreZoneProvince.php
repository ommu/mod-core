...
class CoreZoneProvince extends \app\components\ActiveRecord
{
	...
	public function init()
	{
		...
		$this->templateColumns['countryName'] = [
			...
			'visible' => !Yii::$app->request->get('country') ? true : false,
		];
		...
		$this->templateColumns['cities'] = [
			...
			'value' => function($model, $key, $index, $column) {
				...
				return Html::a($cities, ['zone/city/manage', 'province' => $model->primaryKey, 'publish' => 1], ['title' => Yii::t('app', '{count} cities', ['count' => $cities]), 'data-pjax' => 0]);
			},
			'filter' => false,
			'contentOptions' => ['class' => 'text-center'],
			'format' => 'raw',
