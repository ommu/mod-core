...
class PhraseController extends Controller
{
				...
				if(isset($_GET['enablesave']) && $_GET['enablesave'] == 1) {
					if($model->save()) {
						echo CJSON::encode(array(
							...
							'id' => 'partial-source-message',
							'msg' => '<div class="errorSummary success"><strong>'.Yii::t('phrase', 'Source message success created.').'</strong></div>',
						));
						/*
						Yii::app()->user->setFlash('success', Yii::t('phrase', 'Source message success created.'));
						$this->redirect(array('manage'));
						*/
					} else
				...
				if(isset($_GET['enablesave']) && $_GET['enablesave'] == 1) {
					if($model->save()) {
						echo CJSON::encode(array(
							...
							'get' => Yii::app()->controller->createUrl('manage'),
							'id' => 'partial-source-message',
							'msg' => '<div class="errorSummary success"><strong>'.Yii::t('phrase', 'Source message success updated.').'</strong></div>',
						));
						/*
						Yii::app()->user->setFlash('success', Yii::t('phrase', 'Source message success updated.'));
						$this->redirect(array('manage'));
						*/
					} else
						print_r($model->getErrors());
	...
	public function actionDelete($id) 
	{
		...
		if(Yii::app()->request->isPostRequest) {
			...
			if($model->delete()) {
				echo CJSON::encode(array(
					...
					'get' => Yii::app()->controller->createUrl('manage'),
					'id' => 'partial-source-message',
					'msg' => '<div class="errorSummary success"><strong>'.Yii::t('phrase', 'Source message success deleted.').'</strong></div>',
				));
				/*
				Yii::app()->user->setFlash('success', Yii::t('phrase', 'Source message success deleted.'));
				$this->redirect(array('manage'));
				*/
			}
			Yii::app()->end();
		...
		$this->pageTitle = Yii::t('phrase', 'Delete Phrase: {id}', array('{id}'=>$model->id));
		$this->pageDescription = '';
		$this->pageMeta = '';
